 ...
 * timestamp
 * */
inline void EncodeTime(uint64_t timestamp, ULID& ulid) {
 ...
 * */
inline void Encode(uint64_t timestamp, const std::function<uint8_t()>& rng, ULID& ulid) {
 ...
 * */
inline ULID Create(uint64_t timestamp, const std::function<uint8_t()>& rng) {
	ULID ulid;
...
inline ULID CreateUnique(uint64_t timestamp) {
	ULID ulid;
	EncodeTime(timestamp,ulid);
	EncodeEntropyRand(ulid);
	return ulid;
 ...
 * */
inline uint64_t Time(const ULID& ulid) {
	uint64_t ans = 0;
